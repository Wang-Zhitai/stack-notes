...
* 在驱动程序中如何响应用户空间发起的系统调用，比如用户空间调用了 `open()` 函数，驱动中也要有个 `open()` 函数与之对应，。每一个系统调用，在驱动中都有与之对应的一个驱动函数，在Linux内核文件 `include/linux/fs.h `中有一个叫做**文件操作集**的结构体 `file_operations` ，此结构体就是Linux内核驱动操作函数的集合。
