...
1. 首先要新建一个虚拟机，`先不安装系统`，为了性能最好，推荐`IO控制器选准虚拟化SCSI`，然后`虚拟磁盘类型选SCSI`，然后选择将`虚拟磁盘储存为单个文件`,`CPU核心数`给到最大，`内存`也给到限定范围内的最大值
2. 打开高级设置将收集调试信息设置为`无`，勾选`禁用内存页面修整`可减少卡顿（非常重要，否则虚拟机会将物理内存中的一些数据同步到硬盘上。读取硬盘是虚拟机运行缓慢的主要原因），勾选`为启用了 Hyper-V 的主机禁用侧通道缓解`。
3. 固件类型选`BIOS`
